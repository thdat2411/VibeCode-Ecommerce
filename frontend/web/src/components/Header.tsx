...
export default function Header() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    // Run an immediate sample — for server-rendered pages the data-nav-theme
    // attribute is already in the DOM before this effect fires.
    triggerThemeUpdate();
    const handleDataLoadComplete = () => {
      triggerThemeUpdate();
    };
    window.addEventListener("dataLoadComplete", handleDataLoadComplete);
    // Short fallback for client-rendered pages that paint slightly later.
    const fallbackTimeout = setTimeout(triggerThemeUpdate, 150);
    return () => {
